...
def llik_gamma(x, s=None, batch_size=64, lr=1e-2, **kwargs):
...
column of x
  ...
  """
  x_csr, x_csc, s, genes, max_epochs = scmodes.benchmark.gof._sgd_prepare(x, s, None, batch_size)
  log_mean, log_inv_disp, _ = scmodes.ebpm.sgd.ebpm_gamma(x_csr, s=s, batch_size=batch_size, lr=lr, max_epochs=max_epochs)
  llik = st.nbinom(n=np.exp(log_inv_disp), p=1 / (1 + s * np.exp(log_mean + log_inv_disp))).logpmf(x).sum(axis=0)
  return pd.DataFrame(llik, columns=['llik'], index=genes)
  
def llik_point_gamma(x, s=None, batch_size=64, lr=1e-2, **kwargs):
  """Return marginal log likelihood of Gamma expression model for each
column of x
  ...
  nb_llik = st.nbinom(n=np.exp(log_inv_disp), p=1 / (1 + s * np.exp(log_mean + log_inv_disp))).logpmf(x)
  case_zero = -np.log1p(np.exp(-logodds)) + np.log1p(np.exp(nb_llik - logodds))
  case_non_zero = -np.log1p(np.exp(logodds)) + nb_llik
  llik = np.where(x < 1, case_zero, case_non_zero).sum(axis=0)
  return pd.DataFrame(llik, columns=['llik'], index=genes)
